...
namespace Listas
{
    public partial class frmCargarOnu : Form
    {
        ...
        {
            InitializeComponent();
            txtRuta.Text = Listas.Properties.Settings.Default.urlOnu;
        ...
        private void frmCargarOfac_Load(object sender, EventArgs e)
        {
            ...
            ultimaPublicacion = Funciones.getLastChargedList("ONU");
        ...
        private void btnCargar_Click(object sender, EventArgs e)
        {
            try
            {
                dtPublicacionFile = Funciones.readONUPublish_Date(txtRuta.Text.Trim());
               if (dtPublicacionFile <= this.ultimaPublicacion)
                ...
                listaOFAC = new BindingList<IbsStructurePlana>(Funciones.converToPlain(Funciones.readIbsFromOnuList(txtRuta.Text.Trim())));
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Funciones.saveList(listaOFAC.ToList(), dtPublicacionFile, "ONU"))
                {
                    MessageBox.Show("La lista ONU ha sido procesada satisfacrtoriamente!!.", "SERFINANSA::.",MessageBoxButtons.OK,MessageBoxIcon.Information);
                    ultimaPublicacion = dtPublicacionFile;
                    lblFechaPublicacion.Text = ultimaPublicacion.ToShortDateString();
                    fnProcesando(false);
                    SaveFileDialog sd = new SaveFileDialog();
                    sd.FileName = "ONU" + dtPublicacionFile.Year + dtPublicacionFile.Month.ToString().PadLeft(2, '0') + dtPublicacionFile.Day.ToString().PadLeft(2, '0');
                    ...
                    frmResumenLista frmResumen = new frmResumenLista("ONU");
